  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      child: Builder(builder: (context) {
        ...
        // - nextRequiredLevel
        final title =
            additional['title'] ?? additional['titulo'] ?? additional['rankTitle'];
        final job =
            additional['job'] ?? additional['profesion'] ?? additional['role'];
        ...
        final levelNumber = additional['level_number'] ??
            additional['level'] ??
            additional['nivel'] ??
            additional['xpLevel'];
        ...
        final num? minExp =
            parseNum(additional['minExpRequired'] ?? additional['minExp']);
        final num? nextReq = parseNum(additional['nextRequiredLevel'] ??
        ...
        try {
          if (totalExp != null && minExp != null && nextReq != null) {
            ...
            if (denom > 0) {
              expRatio = ((totalExp.toDouble() - minExp.toDouble()) / denom)
                  .clamp(0.0, 1.0);
            } else {
              expRatio = 0.0;
            }
            ...
            if (nextReq.toDouble() > 0) {
              expRatio =
                  (totalExp.toDouble() / nextReq.toDouble()).clamp(0.0, 1.0);
            }
          }
        } catch (_) {
          expRatio = null;
